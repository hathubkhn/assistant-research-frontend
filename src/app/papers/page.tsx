...
export default function PapersPage() {
    ...
    const fetchVenues = async () => {
        try {
            ...
            try {
                const countsResponse = await fetch(`${API_URL}/api/venues/counts/`, {
                    ...requestOptions,
            ...
            try {
                ...
                if (conferencesResponse.ok) {
                    ...
                    console.error('Failed to fetch conferences:', conferencesResponse.status, conferencesResponse.statusText);
                    try {
            ...
            try {
                ...
                if (journalsResponse.ok) {
                    ...
                    console.error('Failed to fetch journals:', journalsResponse.status);
                    setJournals([]);
                }
            } catch (error) {
    ...
    const fetchPapers = async (page: number = 1, size: number = pageSize) => {
        try {
            ...
            try {
                const response = await fetch(`${API_URL}/api/papers/?${queryParams}`, {
                    ...requestOptions,
                    signal: controller.signal
                });
                            ...
                            <div className={`space-y-2 ${!expandedFilters.conferences ? 'max-h-60 overflow-y-auto' : ''}`}>
                                {loadingVenues ? (
                                    ...
                                    <>
                                        ...
                                        <p className="text-xs text-gray-400">Try clearing your search or check network connection</p>
                                    </>
                                )}
                                                ...
                                                >
                                                    <div className="bg-white p-6 rounded-xl shadow-sm hover:shadow-md transition-shadow">
                                                        <div className="flex justify-between">
                                                            <h2 className="text-xl font-semibold text-gray-800 mb-2 hover:text-hust-red hover:underline">
                                                                {paper.title}
                                                            </h2>
                            ...
                            <div className="flex justify-center items-center mt-8 space-x-4">
                                <div className="flex items-center space-x-1">
                                    ...
                                    <button
                                        ...
                                        className={`px-4 py-2 rounded-md ${currentPage === totalPages || totalPages === 0 ? 'text-gray-400 cursor-not-allowed' : 'text-hust-red hover:bg-red-100'}`}
                                    >
                                        Next
                                    </button>
                                </div>
