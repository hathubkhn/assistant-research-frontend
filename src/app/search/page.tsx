...
function SearchPageContent() {
    ...
    useEffect(() => {
        if (queryParam) {
            performSearch(queryParam, typeParam, parseInt(pageParam, 10), parseInt(sizeParam, 10));
        }
    }, [queryParam, typeParam, pageParam, sizeParam]);

    const performSearch = async (query: string, type: string, page: number = 1, size: number = pageSize) => {
        setLoading(true);
        setError('');

        try {
            const response = await fetch(`${API_URL}/api/search?q=${encodeURIComponent(query)}&type=${type}&page=${page}&pageSize=${size}`);
    ...
    const handleSearch = (e: React.FormEvent) => {
        ...
        router.push(`/search?query=${encodeURIComponent(searchQuery)}&type=${searchType}&page=1&pageSize=${pageSize}`);
    ...
    const handleTypeChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        if (searchQuery.trim()) {
            router.push(`/search?query=${encodeURIComponent(searchQuery)}&type=${newType}&page=1&pageSize=${pageSize}`);
    ...
    const handlePageChange = (newPage: number) => {
        ...
        router.push(`/search?query=${encodeURIComponent(searchQuery)}&type=${searchType}&page=${newPage}&pageSize=${pageSize}`);
    ...
    const handleItemsPerPageChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        router.push(`/search?query=${encodeURIComponent(searchQuery)}&type=${searchType}&page=1&pageSize=${newSize}`);
                ...
                <div className="bg-white rounded-lg shadow-md overflow-hidden p-6 mb-8">
                    ...
                    {!loading && !error && (
                        <div>
                            {results.length > 0 ? (
                                <div>
                                    <h2 className="text-lg font-semibold mb-4">Kết quả tìm kiếm ({totalItems})</h2>
                                    ...
                                    <div className="space-y-6">
                                        {results.map((result) => {
                                            if (result.type === 'user') {
                                                ...
                                                return (
                                                    <div key={`user-${result.id}`} className="flex border-l-4 border-blue-500 bg-white shadow-sm p-4 rounded-r-md">
                                                        ...
                                                        <div>
                                                            <Link href={`/profile/${result.username}`} className="text-blue-600 hover:underline">
                                                                <h3 className="text-lg font-medium">{result.full_name}</h3>
                                                            </Link>
                                                            <p className="text-gray-600">{result.position} - {result.faculty_institute}</p>
                                                            <p className="text-gray-500 mt-1 text-sm">{result.keywords}</p>
                                                        </div>
                                                    </div>
                                                );
                                            } else {
                                                ...
                                                return (
                                                    <div key={`paper-${result.id}`} className="border-l-4 border-green-500 bg-white shadow-sm p-4 rounded-r-md">
                                                        ...
                                                        <p className="text-gray-500 mt-1">
                                                            <span className="text-gray-700">{result.journal}</span> ({result.year})
                                                        </p>
                                                    </div>
                                                );
                                            }
...
export default function SearchPage() {
    return (
        <Suspense fallback={<div className="min-h-screen bg-gray-100 py-8 flex justify-center items-center">
            <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-blue-500"></div>
        </div>}>
            <SearchPageContent />
        </Suspense>
