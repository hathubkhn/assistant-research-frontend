...
export default function MyLibraryPage() {
        ...
        try {
            ...
            if (response.ok) {
                ...
                const interestingDatasets = dataArray.map((dataset: any) => ({
                    id: dataset.id,
                    name: dataset.name,
                    abbreviation: dataset.abbreviation || dataset.name.split(' ').map((word: string) => word[0]).join('').toUpperCase(),
    ...
    const fetchRecommendedPapers = async () => {
        ...
        console.log("Fetching recommended papers for library page... (Called from " + activeSection + " section)");
        ...
        try {
            ...
            const paperObjects = recommendedPapersData.map((paper: any) => {
                return {
                    ...
                    conference: paper.venue_name || paper.venue || paper.conference || "Unknown",
                    year: paper.year || new Date().getFullYear(),
            ...
            paperObjects.sort((a: Paper, b: Paper) =>
    ...
    const togglePaperStatus = async (paperId: string, statusType: 'isInteresting' | 'isDownloaded') => {
        try {
            const API_URL = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000';
            const endpoint = statusType === 'isInteresting'
    ...
    const deleteDownloadedPaper = async (paperId: string, e: React.MouseEvent) => {
    ...
    const toggleStar = async (e: React.MouseEvent, paper: Paper) => {
        ...
        try {
            ...
            if (!paper.isInteresting) {
                ...
                setLibraryPapers(prev =>
                    prev.map(p => p.id === paper.id ? { ...p, isInteresting: false } : p)
                );
                ...
                if (activeSection === 'recommended') {
                    ...
                    const paperInRecommendations = recommendedPapers.some(p => p.id === paper.id);
                            ...
                            <div className="bg-white shadow overflow-hidden sm:rounded-lg">
                                <div className="px-4 py-5 sm:px-6 flex justify-between items-center">
                                    <h2 className="text-lg leading-6 font-medium text-gray-900">
                                        {activeSection === 'interesting' ? t('interestingPapers') :
                                            activeSection === 'downloaded' ? t('downloadedPapers') :
                                                activeSection === 'uploaded' ? t('uploadedPapers') :
                                                    t('interestingDatasets')}
                                    </h2>
                                ...
                                <div className="px-4 text-sm text-gray-600 mb-4 flex justify-between items-center">
                                    <div>
                                        {activeSection !== 'datasets' ? (
                                            <>
                                                {t('results.showing')} {filteredPapers.length} {filteredPapers.length === 1 ? t('results.result') : t('results.results')}
                                            ...
                                            <>
                                                {t('results.showing')} {filteredDatasets.length} {filteredDatasets.length === 1 ? t('results.dataset') : t('results.datasets')}
                                            ...
                                            <div className="grid gap-6">
                                                {filteredPapers.length > 0 ? (
                                                            ...
                                                            <div className="bg-white p-6 rounded-xl shadow-sm hover:shadow-md transition-shadow border border-gray-100">
                                                                <div className="flex justify-between">
                                                                    ...
                                                                    <div className="flex items-center space-x-1">
                                                                        <span
                                                                            onClick={(e) => navigateToConference(getVenueId(paper.venue || paper.conference), e)}
                                                                        ...
                                                                        <span className="text-xs font-medium text-gray-600">{paper.year}</span>
                                                                    </div>
                                                                </div>

                                                                <p className="text-sm text-gray-600 mb-2">
                                                                    <span className="font-medium">{t('papers.authors')}</span> {Array.isArray(paper.authors) ? paper.authors.join(', ') : (paper.authors || '')}
                                                                </p>
                                                                ...
                                                                <div className="flex justify-between items-center">
                                                                    ...
                                                                    <div className="flex space-x-2">
                                                                        {paper.isDownloaded && (
                                                                            <span className="inline-flex items-center text-xs text-green-600">
                                                                                ...
                                                                                {activeSection === 'downloaded' && (
                                                                                    <button
                                                                                        onClick={(e) => deleteDownloadedPaper(paper.id, e)}
                                                                        ...
                                                                        <span className="text-xs text-gray-500">
                                                                            {t('papers.added')} {new Date(paper.addedDate).toLocaleDateString()}
                                                                        </span>
                                                                        {paper.fileName && (
                                                                            <span className="text-xs text-gray-500">
                                                    ...
                                                    <div className="text-center py-12">
                                                        ...
                                                        <h3 className="mt-2 text-sm font-medium text-gray-900">{t('papers.noPapersFound')}</h3>
                                        ...
                                        <div className="grid gap-6">
                                            {filteredDatasets.length > 0 ? (
                                                        ...
                                                        <div className="bg-white p-6 rounded-xl shadow-sm hover:shadow-md transition-shadow border border-gray-100">
                                                            ...
                                                            <div className="flex justify-between items-center">
                                                                ...
                                                                <div className="flex space-x-2">
                                                                    ...
                                                                    <span className="text-xs text-gray-500">
                                                                        {t('papers.added')} {new Date(dataset.addedDate).toLocaleDateString()}
                                                                    </span>
                                                                </div>
                                                            </div>
                                                        </div>
                                                ...
                                                <div className="text-center py-12">
                                                    ...
                                                    <h3 className="mt-2 text-sm font-medium text-gray-900">{t('papers.noDatasetsFound')}</h3>
                                                    <p className="mt-1 text-sm text-gray-500">
                                                        {t('noItems.datasets')}
                                                    </p>
                                                </div>
                                            ...
                                            <div className="space-y-4">
                                                {recommendedPapers.length > 0 ? (
                                                    recommendedPapers.map((paper) => (
                                                        <div key={paper.id} className="bg-white p-4 rounded-lg shadow-sm hover:shadow-md transition-shadow border border-gray-100">
                                                            ...
                                                            <div className="mt-1 text-xs text-gray-500">
                                                                {paper.conference || paper.venue || "Unknown Venue"}
                                                            </div>
                                                    ...
                                                    <div className="text-center py-8">
                                                        ...
                                                        <h3 className="mt-2 text-sm font-medium text-gray-900">{t('papers.noRecommendations')}</h3>
                ...
                <div className="fixed inset-0 overflow-y-auto z-50 flex items-center justify-center"
                    ...
                    <div className="bg-white rounded-lg shadow-xl p-6 m-4 max-w-2xl w-full">
                        ...
                        <p className="text-sm text-gray-600 mb-4">
                            <span className="font-medium">{t('papers.authors')}</span> {Array.isArray(selectedPaper.authors) ? selectedPaper.authors.join(', ') : (selectedPaper.authors || '')}
                        </p>

                        <div className="mb-4">
                            <h4 className="text-sm font-medium text-gray-900 mb-1">{t('papers.abstract')}</h4>
                            <p className="text-sm text-gray-700">{selectedPaper.abstract}</p>
                        </div>

                        {selectedPaper.doi && (
                            <div className="mb-2">
                                <span className="text-sm font-medium text-gray-900">{t('papers.doi')}: </span>
                        ...
                        {selectedPaper.sourceCode && (
                            <div className="mb-2">
                                <span className="text-sm font-medium text-gray-900">{t('papers.sourceCode')}: </span>
                        ...
                        {Array.isArray(selectedPaper.datasets) && selectedPaper.datasets.length > 0 && (
                            <div className="mb-2">
                                <span className="text-sm font-medium text-gray-900">{t('papers.datasets')}: </span>
                                <span className="text-sm text-gray-700">{selectedPaper.datasets.join(', ')}</span>
                            </div>
                        )}

                        <div className="mb-2 mt-4">
                            <span className="text-sm font-medium text-gray-900">{t('papers.addedDate')}: </span>
                            <span className="text-sm text-gray-700">{new Date(selectedPaper.addedDate).toLocaleString()}</span>
                        </div>
                ...
                <div className="fixed inset-0 overflow-y-auto z-50 flex items-center justify-center"
                    ...
                    <div className="bg-white rounded-lg shadow-lg max-w-md w-full p-6">
                        <div className="flex justify-between items-center mb-4">
                            <h3 className="text-xl font-semibold text-gray-900">{t('papers.uploadPaper')}</h3>
                            <button
                            ...
                            <div className="mb-4">
                                ...
                                {!isUploading ? (
                                    ...
                                    >
                                        ...
                                        <p className="mt-2 text-sm text-gray-600" dangerouslySetInnerHTML={{ __html: t('papers.dragAndDrop') }} />
                                        <p className="mt-1 text-xs text-gray-500">
