...
const InterestingDatasetButton = ({
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      const interestState = localStorage.getItem(
        `dataset-starred-${datasetId}`,
      )
  ...
  const toggleInteresting = async () => {
    try {
      ...
      if (!response.ok) {
        ...
        const errorMessage =
          errorData.error || 'Failed to toggle interesting state'
      ...
      // Success message
      toast.success(
        newState ? 'Added to your interests' : 'Removed from your interests',
      )
    } catch (error) {
